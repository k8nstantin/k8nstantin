...
I build AI systems on top of data infrastructure, and lately the infrastructure that autonomous agents need in order to be trusted with real work. Four threads, all running at once: **retrieval systems** that answer questions over an organization's own data, an **agentic operating system** that governs and directs coding agents, **autonomous agent development** — agents that build production software end to end — and the **version control** those agents need, because Git was designed for a different kind of author.

**Retrieval.** In production at Gryphon AI: a semantic knowledge network over 17,000+ database objects across 27 databases and 5 servers, summarized by Gemini 2.5 Pro with PII detection, embedded with `text-embedding-005`, indexed in Vertex AI Vector Search, answering natural-language questions with source citations — the full pipeline as one atomic Dataproc Serverless job that rebuilds for about $30 ([detail below](#schema-intelligence--semantic-search-across-your-entire-database)). What building it taught me is where vector retrieval stops working: nearest-neighbour search finds text that reads similar, not entities that are actually related, and the moment a question needs two or three hops of context, embeddings alone return confident nonsense. Which is why the retrieval work since has been graph-shaped — [go-leiden](https://github.com/k8nstantin/go-leiden) for community detection (the algorithm behind Microsoft GraphRAG) and [swindex](https://swindex.ai) as the index that makes multi-hop queries fast enough to sit in a request path. There is also a ground-truth verifier for RAG systems in progress — read the source, interrogate the RAG, log every gap between them — because "the retrieval is good" is a claim that should be measured, not asserted.

**Agentic OS.** Coding agents forget everything between sessions, cannot be asked what they did or what it cost, and report their own completion. [SuperX](https://github.com/k8nstantin/superx) is the substrate that fixes those three things: it captures every agent on the machine from their own transcripts, records the work as an append-only graph, schedules that graph, dispatches agents against it in dependency order, and writes the results back where the next agent finds them. Insert-only throughout — nothing is overwritten, so the history is evidence rather than a story.

**Autonomous agent development.** The point of the OS is agents that ship. [go-leiden](https://github.com/k8nstantin/go-leiden) is the controlled experiment: a production Go library — the first native Leiden implementation in the language — written end to end by autonomous agents with no human code commits, driven through chained specs with a feedback loop that rewrote its own prompt scaffold when tasks stalled. [OpenPraxis](https://github.com/k8nstantin/OpenPraxis) was the platform that ran it, and what it exposed about agent governance is what SuperX was rebuilt to solve.

**Version control for agents.** Git assumes one author at a time, working in sequence; that assumption is what turns concurrent agents into merge conflicts, branch graveyards and hours of reconciliation nobody budgeted for. [KungFu](https://github.com/k8nstantin/kungfu) removes branching entirely: agents splice fine-grained mutations into a single stream and CRDT semantics (Loro) merge them without a reconciliation step, every mutation signed with Ed25519, with "Ghost State" keeping an agent's work mathematically isolated until it is ready to be seen. It exposes an MCP server, so an agent drives version control natively instead of shelling out to a porcelain designed for humans.

**The data foundation under all of it.** Twenty-five years of database and distributed-systems architecture: MySQL/PostgreSQL at scale, synchronous replication and HA, CDC pipelines through Debezium and Kafka, Iceberg lakehouses on GCP, connection pooling and query routing at ProxySQL, and platform work at Oracle, Apple, Walmart eCommerce and O'Reilly. That background is why the agent substrate looks the way it does — SCD-2 chains, computed current state, insert-only audit trails, UUIDv7 keys, indexes designed for the read path. Agent infrastructure is a data problem wearing a new hat, and most of it is being rebuilt right now by people who have not had to run a system where the history had to be correct.
